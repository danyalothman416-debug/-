"""
Commercial Multi-Tenant Debt Management System
Engineered for Production, Data Isolation & Auto-Migrations
...
def init_db():
    with get_db() as conn:
        ...
        # ٢. خشتەی بەکارهێنەران (پەیوەست بە ئیمەیڵی تایبەت)
        cursor.execute("""
...
# چوونەژوورەوە یان خۆتۆمارکردن لە ڕێگەی ئیمەیڵ
...
def email_login():
    ...
    if not email or "@" not in email:
        return jsonify({"status": "error", "message": "تکایە ئیمەیڵێکی دروست بنووسە"}), 400
    ...
    with get_db() as conn:
        ...
        # ئەگەر بەکارهێنەرەکە نوێ بوو، بە خۆکاری ئەکاونت و دوکانی تایبەتی بۆ دروست بکە
        if not user:
            ...
            cursor.execute("""
                INSERT INTO users (business_id, email, password_hash, full_name, role, created_at)
                VALUES (?, ?, ?, ?, 'owner', ?)
            """, (biz_id, email, generate_password_hash(password), email.split("@")[0], datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
            user_id = cursor.lastrowid

            log_audit(conn, biz_id, email, "REGISTER", "دروستکردنی دوکانی نوێ لە ڕێگەی ئیمەیڵ")
            conn.commit()

            user_data = {
                "id": user_id, "email": email, "full_name": email.split("@")[0], "role": "owner",
            ...
            log_audit(conn, user["business_id"], user["email"], "LOGIN", "چوونەژوورەوەی ئاسایی")
            conn.commit()
...
# چوونەژوورەوەی ڕاستەوخۆ بە یەک کلیک لە ڕێگەی Google
...
def google_one_click():
    ...
    if not email or "@" not in email:
        return jsonify({"status": "error", "message": "ئیمەیڵی گووگڵ نەدۆزرایەوە"}), 400
    ...
    with get_db() as conn:
        ...
        if not user:
            ...
            cursor.execute("""
                INSERT INTO users (business_id, email, full_name, avatar, role, created_at)
                VALUES (?, ?, ?, ?, 'owner', ?)
            """, (biz_id, email, full_name, avatar, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
            user_id = cursor.lastrowid
            conn.commit()
            ...
            user_data = {"id": user["id"], "email": user["email"], "full_name": user["full_name"], "avatar": user["avatar"], "bio": user["bio"]}
            biz_data = {"id": user["business_id"], "name": user["business_name"], "expires_at": user["expires_at"]}
...
def update_profile():
    ...
    with get_db() as conn:
        ...
        log_audit(conn, session["business"]["id"], session["user"]["email"], "UPDATE_PROFILE", "نوێکردنەوەی پرۆفایل")
        conn.commit()
...
def create_debt():
    ...
    with get_db() as conn:
        ...
        log_audit(conn, biz_id, session["user"]["email"], "ADD_DEBT", f"قەرزی نوێ بۆ {name} بە بڕی {amount} {currency}")
        conn.commit()

    return jsonify({"status": "success", "message": "قەرزەکە بە سەرکەوتوویی پاشەکەوت کرا"})
...
def make_payment(debt_id):
    ...
    with get_db() as conn:
        ...
        conn.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (biz_id, debt_id, receipt_no, amount, datetime.now().strftime("%Y-%m-%d %H:%M"), note, session["user"]["email"]))

        update_debt_status(conn, biz_id, debt_id)
        log_audit(conn, biz_id, session["user"]["email"], "PAYMENT", f"وەرگرتنی {amount} {debt['currency']} بۆ وەسڵی #{receipt_no}")
        conn.commit()
...
def delete_debt(debt_id):
    ...
    with get_db() as conn:
        ...
        if debt:
            ...
            log_audit(conn, biz_id, session["user"]["email"], "DELETE_DEBT", f"سڕینەوەی قەرزی {debt['customer_name']}")
            conn.commit()
    return jsonify({"status": "success"})
