...
import pytz # کتێبخانەی تایبەت بە کاتی وڵاتان

# --- 1. ڕێکخستنی لاپەڕە و کاتی بەغداد ---
st.set_page_config(page_title="Golden Delivery", layout="wide")

# ڕێکخستنی کات بەپێی ئاسیای بەغداد
baghdad_tz = pytz.timezone('Asia/Baghdad')
now_bgd = datetime.now(baghdad_tz)
current_time = now_bgd.strftime("%Y-%m-%d | %I:%M %p")
...
st.markdown("""
    ...
    .brand-desc { color: #ffffff; font-size: 16px; margin-top: 10px; line-height: 1.6; }
    .live-clock {
        background-color: rgba(212, 175, 55, 0.1);
        color: #1a1a1a; padding: 10px; border-radius: 10px;
        text-align: center; font-weight: bold; margin-bottom: 15px;
        border: 1px solid #D4AF37; font-size: 18px;
    }
...
# نیشاندانی کاتی بەغداد لە سەرەوەی فۆرمەکە
st.markdown(f"""
    <div class="live-clock">
        🕒 کات و بەرواری ئێستا (بەغداد): {current_time}
    </div>
""", unsafe_allow_html=True)
...
with st.form("delivery_form", clear_on_submit=True):
    ...
    if submit:
        if not customer or not shop_name or not phone:
            ...
            df = load_data()
            # کاتی تۆمارکردنی وەسڵەکە بە کاتی بەغداد
            timestamp = datetime.now(baghdad_tz).strftime("%Y-%m-%d %H:%M")
            new_row = pd.DataFrame([{
                "کات": timestamp, "کڕیار": customer, "ناوی دوکان": shop_name, 
...
# --- ٤. بەشی کارگێڕی (Admin) ---
with st.expander("🛠 بەشی کارگێڕی"):
    if st.text_input("کۆدی نهێنی", type="password", key="admin_pwd") == ADMIN_PASSWORD:
        df_admin = load_data()
        c1, c2, c3 = st.columns(3)
        with c1: st.markdown(f'<div class="stat-card">📦 وەسڵەکان<br>{len(df_admin)}</div>', unsafe_allow_html=True)
        with c2: st.markdown(f'<div class="stat-card">💰 کۆی پارە<br>{df_admin["نرخ"].sum():,}</div>', unsafe_allow_html=True)
        with c3: st.markdown(f'<div class="stat-card">🏪 دوکانەکان<br>{df_admin["ناوی دوکان"].nunique()}</div>', unsafe_allow_html=True)
...
st.markdown("""<div class="install-bar">بۆ دابەزاندنی ئەپ: کلیک لە ⎙ یان ⋮ بکە و Add to Home Screen هەڵبژێرە</div>""", unsafe_allow_html=True)
