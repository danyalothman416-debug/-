...
# ================================
DISEASE_DATABASE = {
    "شەکرەی جۆری 2": {"نیشانەکان": ["تینوویەتی زۆر", "میزی زۆر", "ماندوویی"], "ئاستی مەترسی": "مەترسیدار"},
    "پەستانی خوێنی بەرز": {"نیشانەکان": ["سەرئێشە", "سەرگێژخواردن"], "ئاستی مەترسی": "مامناوەند"},
}

DEFAULT_LABS = {
    "CBC": {"گروپ": "خوێن", "نۆرماڵ": "4.0-11.0", "یەکە": "x10³/µL", "تەفسیر": "خڕۆکە سپیەکان", "ئامێر": "سێل کاونتر"},
    "Glucose": {"گروپ": "بایۆکیمیایی", "نۆرماڵ": "70-126", "یەکە": "mg/dL", "تەفسیر": "شەکری خوێن", "ئامێر": "گلوکۆمیتەر"},
}
...
# ================================
# 3. یارمەتیدەرەکان بۆ UI
# ================================
def get_lab_list(user_data):
    return {**DEFAULT_LABS, **user_data.get("custom_lab_tests", {})}

def get_drug_list(user_data):
    all_drugs = {**DEFAULT_DRUGS}
    all_drugs.update(user_data.get("custom_drugs", {}))
    return all_drugs

# ================================
# 4. دروستکردنی ئەپلیکەیشنەکە (Flet)
# ================================
def main(page: ft.Page):
    ...
    # ================================
    # 5. فانکشنەکانی نیشاندانی پەڕەکان (Views)
    # ================================
    ...
    def show_login_view(e=None):
        ...
        def do_register(e):
            ...
            if create_user(username_input.value, password_input.value):
                page.snack_bar = ft.SnackBar(ft.Text("هەژمارەکەت دروست کرا! ئێستا بچۆ ژوورەوە."), bgcolor=ft.colors.GREEN_800)
                page.snack_bar.open = True
                page.update()
            else:
        ...
        page.add(
            ft.Container(
                ...
                gradient=ft.LinearGradient(
                    ...
                    colors=[ft.colors.BLUE_900, ft.colors.GREY_900]
                )
            )
        )
        page.update()
    ...
    def show_dashboard_view(e=None):
        ...
        stats = [
            ...
            ft.Card(content=ft.Container(content=ft.Column([ft.Icon(ft.icons.MEDICATION, size=40, color=ft.colors.GREEN_400), ft.Text(f"{len(get_drug_list(app_state['user_data']))}", size=24, weight=ft.BOLD), ft.Text("دەرمان")], horizontal_alignment=ft.CrossAxisAlignment.CENTER), padding=10), expand=True),
            ft.Card(content=ft.Container(content=ft.Column([ft.Icon(ft.icons.SCIENCE, size=40, color=ft.colors.PURPLE_400), ft.Text(f"{len(get_lab_list(app_state['user_data']))}", size=24, weight=ft.BOLD), ft.Text("پشکنین")], horizontal_alignment=ft.CrossAxisAlignment.CENTER), padding=10), expand=True),
        ]
        ...
        page.add(
            ft.Column([
                ...
                ft.Container(
                    content=ft.Row([
                        ft.Text(f"بەخێربێیت {app_state['username']}", size=18, weight=ft.BOLD),
                ...
                ft.ListTile(
                    leading=ft.Icon(ft.icons.SCIENCE, color=ft.colors.PURPLE_400),
                    title=ft.Text("تاقیگە و پشکنین"),
                    subtitle=ft.Text("زیادکردن، دەستکاری و سڕینەوەی پشکنین"),
                    on_click=show_labs_view,
                    trailing=ft.Icon(ft.icons.CHEVRON_RIGHT)
                ),
                ft.ListTile(
                    leading=ft.Icon(ft.icons.MEDICATION, color=ft.colors.GREEN_400),
                    title=ft.Text("فارماکۆلۆجی"),
                    subtitle=ft.Text("دەرمانەکان و تێبینییەکان"),
                    on_click=show_drugs_view,
                    trailing=ft.Icon(ft.icons.CHEVRON_RIGHT)
                ),
                ft.ListTile(
                    leading=ft.Icon(ft.icons.MEDICAL_INFORMATION, color=ft.colors.RED_400),
                    title=ft.Text("نەخۆشییەکان"),
                    subtitle=ft.Text("زانیاری تەواو لەسەر نەخۆشییەکان"),
                    on_click=show_diseases_view,
                    trailing=ft.Icon(ft.icons.CHEVRON_RIGHT)
                ),
    ...
    def show_labs_view(e=None):
        page.controls.clear()
        all_labs = get_lab_list(app_state["user_data"])
        ...
        def open_add_lab_dialog(e):
            ...
            group_field = ft.TextField(label="گروپ", text_align=ft.TextAlign.RIGHT)
            normal_field = ft.TextField(label="نۆرماڵ (وەک: 70-126)", text_align=ft.TextAlign.RIGHT)
            unit_field = ft.TextField(label="یەکە", text_align=ft.TextAlign.RIGHT)
            ...
            def save_new_lab(e):
                if name_field.value:
                    app_state["user_data"].setdefault("custom_lab_tests", {})[name_field.value] = {
                        "گروپ": group_field.value or "گشتی",
                        "نۆرماڵ": normal_field.value or "نەزانراو",
                        "یەکە": unit_field.value or "",
                        "تەفسیر": "",
                    ...
                    dialog.open = False
                    page.update()
                    show_labs_view() # Refresh view

            dialog = ft.AlertDialog(
                title=ft.Text("زیادکردنی پشکنینی نوێ"),
                content=ft.Column([name_field, group_field, normal_field, unit_field, machine_field, note_field], tight=True, scroll=ft.ScrollMode.AUTO),
                actions=[
                    ft.TextButton("هەڵوەشاندنەوە", on_click=lambda _: close_dialog(dialog)),
                    ft.ElevatedButton("خەزنکردن", on_click=save_new_lab, bgcolor=ft.colors.BLUE_400, color=ft.colors.WHITE)
                ],
            ...
            lab_list_controls.append(
                ft.Card(
                    content=ft.Container(
                        content=ft.Column([
                            ft.Row([
                                ft.Text(name, size=16, weight=ft.BOLD, color=ft.colors.BLUE_200),
                                ft.Container(
                                    content=ft.Text("تایبەت بە من", size=10, color=ft.colors.WHITE),
                                    bgcolor=ft.colors.GREEN_700,
                                    padding=5, border_radius=5,
                                    visible=is_custom
                                )
                            ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
                            ft.Text(f"گروپ: {info.get('گروپ', '')} | ئامێر: {info.get('ئامێر', '')}", size=12, color=ft.colors.GREY_400),
                            ft.Text(f"نۆرماڵ: {info.get('نۆرماڵ', '')} {info.get('یەکە', '')}", size=14, color=ft.colors.WHITE),
                            ft.Text(f"📝 {info.get('تێبینی', 'بێ تێبینی')}", size=12, color=ft.colors.GREY_300, italic=True),
                            ft.Row(
                                [
                                    ft.IconButton(ft.icons.DELETE, icon_color=ft.colors.RED_400, on_click=lambda e, n=name: delete_lab(n), visible=is_custom),
                                ],
        ...
        page.add(
            ft.Column([
                ft.Row([
                    ...
                    ft.Text("تاقیگەی پشکنین", size=22, weight=ft.BOLD),
                ], alignment=ft.MainAxisAlignment.START),
                ft.ElevatedButton("➕ زیادکردنی پشکنینی نوێ", on_click=open_add_lab_dialog, width=ft.width, bgcolor=ft.colors.PURPLE_400, color=ft.colors.WHITE),
                ft.Divider(),
    ...
    def show_drugs_view(e=None):
        page.controls.clear()
        all_drugs = get_drug_list(app_state["user_data"])
        ...
        def open_add_drug_dialog(e):
            ...
            dialog = ft.AlertDialog(
                ...
                actions=[
                    ft.TextButton("هەڵوەشاندنەوە", on_click=lambda _: close_dialog(dialog)),
                    ft.ElevatedButton("خەزنکردن", on_click=save_new_drug, bgcolor=ft.colors.GREEN_400, color=ft.colors.WHITE)
                ],
                actions_alignment=ft.MainAxisAlignment.END,
            )
            page.dialog = dialog
            ...
            drug_list_controls.append(
                ft.Card(
                    content=ft.Container(
                        content=ft.Column([
                            ft.Row([
                                ft.Icon(ft.icons.MEDICATION, color=ft.colors.GREEN_400),
                                ft.Text(name, size=16, weight=ft.BOLD, expand=True),
                                ft.Container(
                                    content=ft.Text("تایبەت بە من", size=10, color=ft.colors.WHITE),
                                    bgcolor=ft.colors.GREEN_700,
                                    padding=5, border_radius=5,
                            ...
                            ft.Row(
                                [
                                    ft.IconButton(ft.icons.DELETE, icon_color=ft.colors.RED_400, on_click=lambda e, n=name: delete_drug(n), visible=is_custom),
                                ],
                                alignment=ft.MainAxisAlignment.END
                            )
                        ]),
        ...
        page.add(
            ft.Column([
                ft.Row([
                    ft.IconButton(ft.icons.ARROW_BACK, on_click=go_back),
                    ft.Text("فارماکۆلۆجی و دەرمان", size=22, weight=ft.BOLD),
                ], alignment=ft.MainAxisAlignment.START),
                ft.ElevatedButton("➕ زیادکردنی دەرمانی نوێ", on_click=open_add_drug_dialog, width=ft.width, bgcolor=ft.colors.GREEN_400, color=ft.colors.WHITE),
                ft.Divider(),
    ...
    def show_diseases_view(e=None):
                    ...
                    content=ft.Container(
                        content=ft.ExpansionTile(
                            ...
                            controls=[
                                ft.Container(
                                    content=ft.Column([
                                        ft.Text("نیشانەکان:", weight=ft.BOLD, color=ft.colors.BLUE_200),
                                        ft.Text("، ".join(info['نیشانەکان'])),
                                    ]),
        ...
        page.add(
            ft.Column([
                ft.Row([
                    ft.IconButton(ft.icons.ARROW_BACK, on_click=go_back),
                    ft.Text("کتیبخانەی نەخۆشییەکان", size=22, weight=ft.BOLD),
                ], alignment=ft.MainAxisAlignment.START),
                ft.Divider(),
