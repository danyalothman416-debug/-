...
class LicenseSystem:
    ...
    def activate_license(self, license_key, device_id):
        ...
        # Check expiration
        expires_str = license_data[5]  # expires_at is at index 5
        try:
            ...
            for fmt in ['%Y-%m-%d %H:%M:%S', '%Y-%m-%dT%H:%M:%S', '%Y-%m-%d']:
                ...
                # If no format works, assume it's not expired
            ...
            # If date parsing fails, assume valid (for safety with existing licenses)
            pass
        ...
        c.execute("UPDATE licenses SET device_id=?, last_used=? WHERE license_key=?",
                 (device_id, datetime.now().strftime('%Y-%m-%d %H:%M:%S'), license_key))
        conn.commit()
    ...
    def log_attempt(self, license_key, device_id, status):
        ...
        c.execute("INSERT INTO activation_attempts (license_key, device_id, attempt_time, status) VALUES (?, ?, ?, ?)",
                 (license_key, device_id, datetime.now().strftime('%Y-%m-%d %H:%M:%S'), status))
        conn.commit()
        conn.close()
