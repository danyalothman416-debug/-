...
def init_db():
    with get_db() as conn:
        ...
        # دروستکردنی هەژماری بنەڕەتیی بەڕێوەبەر
    ...
    with get_db() as conn:
        ...
        if not user:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, 'staff', ?, 'هەژماری بەستراوە بە گووگڵ', ?)
            """, (username, full_name, email, avatar, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
            user_id = cursor.lastrowid
            role, bio = "staff", "هەژماری بەستراوە بە گووگڵ"
        else:
...
def update_profile():
    ...
    with get_db() as conn:
        ...
        log_audit(conn, session["user"]["username"], "UPDATE_PROFILE", "دەستکاریکردنی پرۆفایل و وێنە")
        conn.commit()
