...
# --- 6. بنکەدراوەی دەوڵەمەندی پشکنینەکان (بە زانیاری تەواوەتی) ---
full_lab_data = {
...
# --- 11. وەرگێڕی زیرەکی ئەنجامەکان (AI Interpreter) ---
with st.expander("11. وەرگێڕی زیرەکی ئەنجامەکان (AI Interpreter)"):
    st.markdown('<div class="info-box">جۆری پشکنینەکە هەڵبژێرە و ژمارەی ئەنجامەکە بنووسە بۆ شیکردنەوەی خێرا:</div>', unsafe_allow_html=True)
...
with st.expander("12. چاودێری گەشەی پشکنینەکان (Results Tracker)"):
    st.markdown('<div class="info-box">ئەنجامی پشکنینەکانی پێشووت تۆمار بکە بۆ بینینی گراف و گۆڕانکارییەکان.</div>', unsafe_allow_html=True)
    ...
    v_in = st.number_input("ئەنجام (ژمارە):", value=0.0, key="v12")
    if st.button("تۆمارکردنی ئەنجام"):
        new_row = pd.DataFrame({"Month": [m_in], "Value": [v_in]})
...
with st.expander("13. پشکنینەکان بەپێی نیشانەکان (Symptom Guide)"):
    ...
    symp = st.selectbox("هەست بە چ نیشانەیەک دەکەیت؟", ["ماندوێتی زۆر", "کێشەی هەرس", "ئازاری جومگە"], key="s13")
