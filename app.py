...
DRUG_DATABASE = {
    ...
    "دژە ئەنیمیا": {
        ...
        "فۆلیک اسید": {"ڕێژە": "1-5mg", "میکانیزم": "Folate", "کاریگەری لاوەکی": "کەم", "پێچەوانە": "هەستیاری", "وەسف": "پڕکەری فۆلیک ئەسید", "بۆچی": "بۆ ئەنیمیای ماکرۆسایتیک", "تێبینی": ""},
    ...
    "دژە ئازار": {
        ...
        "پێتیدین": {"ڕێژە": "50mg", "میکانیزم": "Opioid", "کاریگەری لاوەکی": "سەرگێژخواردن", "پێچەوانە": "نەخۆشی دڵ", "وەسف": "دژە ئازاری بەهێز", "بۆچی": "بۆ ئازاری توند", "تێبینی": ""},
        "ناکسۆکسان": {"ڕێژە": "5-10mg", "میکانیزم": "Opioid", "کاریگەری لاوەکی": "سکچوون", "پێچەوانە": "نەخۆشی دڵ", "وەسف": "دژە ئازار", "بۆچی": "بۆ ئازاری ناوەند", "تێبینی": ""},
        "فوێنتانیل": {"ڕێژە": "25mcg", "میکانیزم": "Opioid", "کاریگەری لاوەکی": "خەوی", "پێچەوانە": "نەخۆشی هەناسە", "وەسف": "دژە ئازاری زۆر بەهێز", "بۆچی": "بۆ ئازاری شێرپەنجە", "تێبینی": ""}
    },
...
# ================================
# 15. پەڕەی تاقیگە (٢٠٠ پشکنین) - لەگەڵ توانای نوسینی تێبینی
# ================================
    ...
    for test_name, test_info in all_lab_tests.items():
        ...
        with cols[idx % 2]:
            ...
            # دیاریکردنی کلیل و نرخی سەرەتایی بۆ تێبینی
            is_custom = test_name in st.session_state.custom_lab_tests
            
            if is_custom:
                current_note = test_info.get("تێبینی", "")
                note_key = f"note_custom_lab_{test_name}"
            else:
                current_note = st.session_state.lab_notes.get(test_name, test_info.get("تێبینی", ""))
                note_key = f"note_original_lab_{test_name}"
            ...
            # زیادکردنی text_area بۆ تێبینی
            new_note = st.text_area(
                "📝 تێبینی:",
                value=current_note,
                key=note_key,
                placeholder="تێبینی تایبەتی خۆت لێرە بنووسە...",
                height=68,
                label_visibility="collapsed"
            )
            
            # ئەگەر تێبینییەکە گۆڕا، خەزن بکە
    ...
    with col2:
        if test_to_analyze and test_value:
            ...
            is_custom = test_to_analyze in st.session_state.custom_lab_tests
            if is_custom:
                note = st.session_state.custom_lab_tests[test_to_analyze].get("تێبینی", "")
            else:
                note = st.session_state.lab_notes.get(test_to_analyze, all_lab_tests[test_to_analyze].get("تێبینی", ""))
...
# ================================
# 17. پەڕەی فارماکۆلۆجی - بە توانای نوسینی تێبینی
# ================================
    ...
    if st.session_state.custom_drugs:
        with st.expander(f"📂 دەرمانە تایبەتییەکانی خۆت ({len(st.session_state.custom_drugs)} دەرمان)"):
            ...
            for drug, info in st.session_state.custom_drugs.items():
                with cols[idx % 2]:
                    # تێبینی بۆ دەرمانە کەستەمەکان
                    current_note = info.get("تێبینی", "")
                    note_key = f"note_custom_drug_{drug}"
                    ...
                    if new_note != current_note:
                        st.session_state.custom_drugs[drug]["تێبینی"] = new_note
                        auto_save()
                idx += 1
