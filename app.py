...
Auto-migrating SQLite & Bulletproof API
"""
...
def init_db():
    with get_db() as conn:
        ...
        # پشکنینی ستوونەکانی بەکارهێنەران
        user_cols = [
            ("email", "TEXT"),
            ("avatar", "TEXT DEFAULT 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=150'"),
            ("bio", "TEXT DEFAULT 'بەڕێوەبەری سیستەمی دارایی'"),
        ...
        # پشکنین و نوێکردنەوەی ستوونەکانی خشتەی قەرز لە داتابەیسی کۆندا (Auto Migration)
        debt_cols = [
        ...
        for col, c_type in debt_cols:
            try:
                cursor.execute(f"ALTER TABLE debts ADD COLUMN {col} {c_type}")
            except sqlite3.OperationalError:
                pass
        ...
        """)
        for p_col, p_type in [("receipt_no", "TEXT"), ("received_by", "TEXT")]:
            try:
                cursor.execute(f"ALTER TABLE payments ADD COLUMN {p_col} {p_type}")
            except sqlite3.OperationalError:
                pass
        ...
        # دڵنیابوونەوە لە بوونی ئەدمینی بنەڕەتی
...
# بۆ ئەوەی لە کاتی بەستنەوە بە ئایپی ناوخۆیی سێشن نەمێنێت و بەکارهێنەر ڕانەگرێت
@app.before_request
...
def update_debt_status(conn, debt_id):
    ...
    if remaining <= 0:
        status = "paid"
    elif debt["due_date"] and parse_date_safely(debt["due_date"]) < today:
        status = "overdue"
    elif paid_sum > 0:
        status = "partial"
    else:
...
def api_login():
    ...
    with get_db() as conn:
        ...
        log_audit(conn, user["username"], "LOGIN", "چوونەژوورەوەی ئاسایی")
        conn.commit()
...
def google_auth():
    ...
    if not credential:
        return jsonify({"status": "error", "message": "بڕوانامەی گووگڵ بەردەست نییە"}), 400
    ...
    try:
        ...
        return jsonify({"status": "error", "message": "هەڵە لە زانیارییەکانی گووگڵ"}), 400
    ...
    with get_db() as conn:
        ...
        log_audit(conn, username, "GOOGLE_LOGIN", f"چوونەژوورەوە بە گووگڵ: {email}")
        conn.commit()
...
def update_profile():
    ...
    if not user:
        return jsonify({"status": "error", "message": "پێویستە سەرەتا بچیتە ژوورەوە"}), 401
    ...
    if file and file.filename != '':
        filename = f"avatar_{user['id']}_{int(datetime.now().timestamp())}.{file.filename.rsplit('.', 1)[1].lower()}"
        file_path = os.path.join(app.config["UPLOAD_FOLDER"], filename)
        file.save(file_path)
        avatar_url = f"/static/avatars/{filename}"

    with get_db() as conn:
        conn.execute("UPDATE users SET full_name = ?, bio = ?, avatar = ? WHERE id = ?", (full_name, bio, avatar_url, user["id"]))
        log_audit(conn, user["username"], "UPDATE_PROFILE", "دەستکاریکردنی پرۆفایل")
        conn.commit()
...
# ئەم فەنکشنە بە شێوازی پتەو ڕێکخراوە تا بە هیچ جۆرێک لە کاتی پاشەکەوتکردن سێرڤەر نەپچڕێت
...
def create_debt():
    ...
    debt_date = parse_date_safely(data.get("debt_date"))
    due_date = parse_date_safely(data.get("due_date"))
    note = data.get("note", "").strip()
    is_installment = 1 if str(data.get("is_installment")).lower() in ("true", "1") else 0
    try:
    ...
    try:
        ...
        return jsonify({"status": "error", "message": "بڕی پارە ژمارەیەکی دروست نییە"}), 400
    ...
    with get_db() as conn:
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (name, phone, amount, currency, debt_date, due_date, note, is_installment, installments_count, status, username, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
        debt_id = cursor.lastrowid

        if is_installment and installments_count > 1:
            part_amount = round(amount / installments_count, 2)
            try:
                base_dt = datetime.strptime(debt_date, "%Y-%m-%d")
            except Exception:
                base_dt = datetime.now()
            for i in range(1, installments_count + 1):
                inst_due = (base_dt + timedelta(days=30 * i)).strftime("%Y-%m-%d")
                cursor.execute("""
                    INSERT INTO installments (debt_id, installment_no, amount, due_date, status)
                    VALUES (?, ?, ?, ?, 'pending')
                """, (debt_id, i, part_amount, inst_due))

        log_audit(conn, username, "ADD_DEBT", f"تۆمارکردنی قەرزی {name} بە بڕی {amount} {currency}")
        conn.commit()
...
def make_payment(debt_id):
    ...
    if amount <= 0:
        return jsonify({"status": "error", "message": "بڕی پارەی دراو دەبێت لە ٠ زیاتر بێت"}), 400
    ...
    with get_db() as conn:
        ...
        if amount > remaining_before:
            return jsonify({"status": "error", "message": f"بڕی دراو لە ماوە زۆرترە! ({remaining_before:,.0f} {debt['currency']})"}), 400
        ...
        conn.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        """, (debt_id, receipt_no, amount, datetime.now().strftime("%Y-%m-%d %H:%M"), note, username))

        update_debt_status(conn, debt_id)
        log_audit(conn, username, "PAYMENT", f"وەرگرتنی {amount} {debt['currency']} بۆ وەسڵی #{receipt_no}")
        conn.commit()
    ...
    return jsonify({
        ...
        "receipt": {
            ...
            "received_by": session.get("user", {}).get("full_name", "کارمەند")
        }
    })
...
def delete_debt(debt_id):
    ...
    with get_db() as conn:
        ...
        if debt:
            log_audit(conn, username, "DELETE_DEBT", f"سڕینەوەی قەرزی {debt['customer_name']}")
        conn.commit()
    return jsonify({"status": "success"})
